...
const ImageSlide = ({
    ...
    useEffect(() => {
        const slideCalculateValue = count * 100;
        setTranslateImage(slideCalculateValue);
        if (count > 4) {
            setCount(0)
        }
    }, [count]);
    ...
    return (
        <div className="relative min-w-full ">
            ...
            <div className="flex justify-center gap-2 z-40 relative">
                <p
                    className={`p-1.5 rounded-full bg-slate-200 transition-all cursor-pointer ${count === 0 && "bg-slate-400"
                        }`}
                    onClick={() => directScroll(0)}
                ></p>
                <p
                    className={`p-1.5 rounded-full bg-slate-200 transition-all cursor-pointer ${count === 1 && "bg-slate-400"
                        }`}
                    onClick={() => directScroll(1)}
                ></p>
                <p
                    className={`p-1.5 rounded-full bg-slate-200 transition-all cursor-pointer ${count === 2 && "bg-slate-400"
                        }`}
                    onClick={() => directScroll(2)}
                ></p>
                <p
                    className={`p-1.5 rounded-full bg-slate-200 transition-all cursor-pointer ${count === 3 && "bg-slate-400"
                        }`}
                    onClick={() => directScroll(3)}
                ></p>
                <p
                    className={`p-1.5 rounded-full bg-slate-200 transition-all cursor-pointer ${count === 4 && "bg-slate-400"
                        }`}
